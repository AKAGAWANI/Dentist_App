...
Controller.prototype.refresh = async function (req, res, _next) {
  try {
    ...
    if (userRes) {
      ...
      let userIP = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
...
Controller.prototype.success = async function (req, res, next) {
  try {
    ...
    const userObj = await service.getUserApplications(userId, { accessToken, refreshToken, expiresAt, profileImage });
    userObj.apps = userObj.apps.filter(x => "60929f9509dfffb02e84ef88" !== x.appTypeId);
...
Controller.prototype.local = async function (req, res, next) {
  try {
    let userIP = req.headers.ip || req.ip || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    ...
    passport.authenticate('local', {
      ...
      try {
        ...
        const data = {
          ...
          login_date: (new Date()),
          userIP,
          profileImageURL: user?.social?.google?.picture || user?.social?.facebook?.picture || user?.social?.twitter?.picture
        };
...
Controller.prototype.resetPassword = async function (req, res, next) {
  try {
    ...
    if (!user.userId) {
      res.status(Response.error.NotFound.code).json(Response.error.NotFound.json('No user exists or Invalid OTP'));
      return;
    }
    ...
    logger.error(e.message);
    res.status(Response.error.Forbidden.code).json(Response.error.Forbidden.json(errMsg));
  }
}
  ...
  try {
    passport.authenticate('facebook', {
      ...
      try {
        ...
        return res.redirect(`/login/success?profileImageURL=${url.encodeURI(data.profileImageURL)}&userId=${user._id}&accessToken=${token.access_token}&refreshToken=${token.refresh_token}&expiresAt=${token.expires_at}`);
      } catch (e) {
        return res.status(Response.error.InvalidRequest.code).json(Response.error.InvalidRequest.json());
      }
    })(req, res, next);
